...
后续源码复核与修复见 [2026-09-17 审核记录](REVIEW_2026-09-17.md)。本文后续各轮记录保留其当时的验证范围。
...
## 十八、网格开着，但两种放置方式只有一种吸附（已修）

**来源**：给第十七节补「落点准不准」的断言时，数字自己露出来的。

网格默认是**开的**（`gridEnabled` 初值 `true`）。但同一张画布上：
...
根因是吸附写在了**手势**里而不是**放置**里：`handleDrop` 自己算了一次 `snap`，
而单击走的是 `tap` 分支，直接把 Cytoscape 报的模型坐标透传下去，**完全绕过吸附**。
`placeDrawingNode` 只做了 `Math.round`。

修法：**把吸附从 `handleDrop` 的放置分支移到 `placeDrawingNode`**，
让「放置」这件事自己拥有网格属性 —— 两条路径自然一致。
注册设备的拖放路径仍由 `handleDrop` 吸附（那条路不经过 `placeDrawingNode`）。

### 顺带把断言收紧

原来落点断言用「期望值 ± 18px」—— 因为要容忍吸附带来的半格（16px）偏差。
**这么松的判据看不见小回退，只能抓住它当初要抓的那个 38px。**
改成：网格开时**把期望值也吸附**，然后按 **2px** 判。

| 断言 | 修前 | 修后 |
|---|---|---|
| 单击落点 | 未断言 | 偏差 **0.0px** / 容差 2 |
| 拖放落点 | 未断言 | 偏差 **0.0px** / 容差 2 |
| 两种方式一致 | 单击不在格点 | **都在格点** |

**顺带记一个探针自己的错**：一开始用「坐标是不是 32 的倍数」反推网格开没开，
结果一次真实坐标恰好落在格点上，把容差从 2px 悄悄放宽到 18px ——
**判据自己把自己放松了**。改成从画布的 `grid-on` 类名直接读网格状态。

**教训**：**「同义手势」要断言它们结果一致。** 两种方式做同一件事，
只要实现分叉过，就迟早会在某个属性上不一致；而这个属性往往不是测试关心的那个，
所以只有专门写一条「两者一致」的断言才会发现。
