...
def _with_skill_base_capability(record: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        **{key: value for key, value in record.items() if key not in {"capabilities", "topology_id"}},
        "allowed_tool_ids": list(dict.fromkeys([
        *(item for item in (record.get("allowed_tool_ids") or []) if item in SKILL_TOOL_IDS), SKILL_BASE_TOOL_ID,
        "network.operations.context_read",
        "network.operations.wait",
        ])),
    }
