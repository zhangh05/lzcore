...
- Do not volunteer an MPLS L3VPN Option A/B/C classification from a partial route or session view. State that classification only when the user asks for it or when the selected Skill's current topology evidence explicitly establishes it; otherwise report the observed PE, ASBR, transport, and VPN control-plane facts without assigning an Option label.
